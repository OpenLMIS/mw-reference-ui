...
(function() {
    ...
    function controller($state, $scope, $window, report, reportFactory,
         ...
         */
        function watchDependency(param, relation) {
            var watchProperty = 'vm.selectedParamsOptions.' + relation.dependency;
            $scope.$watch(watchProperty, function(newVal, oldVal) {
                if (newVal && relation.property) {
                    newVal = newVal.dependencies[relation.property];
                }

                vm.selectedParamsDependencies[relation.placeholder] = newVal;
                console.log(newVal);
                if (newVal) {
                    reportFactory.fetchReportParameterOptions(param, vm.selectedParamsDependencies)
                    .then(function(items) {
                        console.log(items);
